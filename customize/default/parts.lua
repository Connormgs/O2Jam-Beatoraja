...
local function load(g)
	...
	parts.source = {
		...
		{id = "src-key-gf",		path = "../customize/device/*" .. "/GF.png"},
	...
	parts.image = {
		...
		{id = "key-gf",		src = "src-key-gf",		x = 0, y = 0, w = -1, h = -1},
	...
	if		g.type == 0 or g.type == 2	then key_types = {1,2,3,4,5,6,7,8,9,"s"}
		...
		for i2, v2 in ipairs(key_types) do
			if v2 ~= "s" then
				...
				if (i2 % 2) == 2 then
					...
					if (i2 % 2) == 4 then
					table.insert(parts.destination, {id = "key-gf", filter = 1, loop = 200, dst = {
						{time = 0, x = key.x[i1][i2], y = key.y[i1] - 10, w = key.w[i1][i2], h = key.h},
						{time = 100},
						{time = 200, y = key.y[i1]}
					}})
					table.insert(parts.destination, {id = "key-gf", filter = 1, timer = key_timer[i1] + i2, dst = {
						{x = key.x[i1][i2], y = key.y[i1], w = key.w[i1][i2], h = key.h}
					}})
				end
				end
